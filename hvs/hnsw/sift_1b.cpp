...
void kmeans_(
	...
	int index;
	int round= 5;  //10
	float** temp_quan = new float*[LLL];  //temp array for
	int* tol_quan = new int[LLL];  //the number of objects on each center
	for(int i = 0; i < LLL; i++)
    ...
    double rand_;
	for(int i = 0; i < LLL; i++){  //generate LLL intial centers randomly
		 rand_ = double(i) / LLL;
		 index = int( (n-1) * rand_);
		if(index < 0 || index >= n){
...
void kmeans0_(
    ...
    double rand_;
	for(int i = 0; i < cen1; i++){  //generate LLL intial centers randomly
		 rand_ = double(i) / cen1;
		 index = int( (n-1) * rand_);
		if(index < 0 || index >= n){
			printf("random_generator error\n");
	...
	float** train0) //size_n * d
{
	...
	int* weight = new int[LLL * LLL];

	for(int i = 0; i < LLL * LLL; i++){
        weight[i] = 0;
	}
	...
	float sum2 = 0;	
	int half_d = d / 2;
	float** array1 = new float* [LLL];
	float** array2 = new float* [LLL];
	for(int i = 0; i < LLL; i++){
	...
	for(int i = 0; i < LLL; i++){
		for(int j = 0; j < half_d; j++){
		    array1[i][j] = train[i * LLL + i][j];
		    array2[i][j] = train[i * LLL + i][half_d + j];
		}
	}	
	for(int i = 0; i < size_n; i++){
		...
		weight[min_id1 * LLL + min_id2]++;
	}
	
	float** temp_quan = new float*[LLL];
	int** count = new int*[LLL];
	int* tol_quan = new int[LLL];
	int* w_quan = new int[LLL];
	
	for(int i = 0; i < LLL; i++){
		temp_quan[i] = new float[d];
	    count[i] = new int[LLL * LLL];
	}
	...
	kmeans0_(train0, result, size_n, d, LLL);
	...
	//---------------------------------------------------------	
	qsort(pointer, LLL, sizeof(int), compare_int);
	//----------------important sampling-------
	...
	//------------------------------------------	
	for(int i = 0; i < LLL; i++){
		merge[2*i] = pointer[i] / LLL;
		merge[2*i+1] = pointer[i] % LLL;
	}
}	
...
void sift_test1B(
	...
	for (int i = 0; i < max_level_; i++){
		for(int ii = 0; ii < length[i]; ii++){
			merge[i][ii] = new unsigned char[2 * LLL];
		}			
	}
	...
	for (int i = 0; i < max_level_; i++){
		...
		for(int ii = 0; ii < length[i]; ii++){
			quantizer[i][ii] = new float* [LLL];
			temp_quan[i][ii] = new float* [LLL * LLL];
		}
		for(int ii = 0; ii < length[i]; ii++){
			for(int j = 0; j < LLL; j++)
				quantizer[i][ii][j] = new float[dim[i]];
			for(int j = 0; j < LLL * LLL; j++)
...
#pragma omp critical
    ...
    CvMat *M_Y;
    M_Y = cvCreateMat(dim_, size_n, CV_32FC1);
	...
	for(int k1 = 0; k1 < ROUND1; k1++){
	    for(int i = 0; i < max_book; i++){
		    for(int k2 = 0; k2 < ROUND2; k2++){
			    ...
			    for(int j = 0; j < LLL; j++) tol_count[j] = 0;
	...
	for (int i = 1; i < max_level_; i++){   //if max_level_ = 1,skip
        ...
        int** con_count = new int*[LLL];
        for(int j = 0; j < LLL; j++)
			con_count[j] = new int[LLL];
		...
		for(int ii = 0; ii < length[i-1]; ii++){
			for(int jj = ii+1; jj < length[i-1]; jj++){
                ...
                for(int kk1 = 0; kk1 < LLL; kk1++ )
                    for(int kk2 = 0; kk2 < LLL; kk2++ )
                        con_count[kk1][kk2] = 0;
				...
				sum3 = 0;
				for(int i1 = 0; i1 < LLL; i1++){
					if( count1[ii][i1] == 0) continue;
					
					for(int j1 = 0; j1 < LLL; j1++){
		...
		for(int ii = 0; ii < length[i]; ii++){
			...
			for(int j1 = 0; j1 < LLL; j1++){
       			for(int j2 = 0; j2 < LLL; j2++){
				    for(int l = 0; l < dim[i]; l++){
						if(l < dim[i-1]) {temp_quan[i][ii][j1 * LLL + j2][l] = quantizer[i - 1][2 * ii][j1][l];}
						else{temp_quan[i][ii][j1 * LLL + j2][l] = quantizer[i - 1][2 * ii + 1][j2][l - dim[i - 1]];}
					}
				}    
			}
	...
	for(int i = 0; i < max_level_ * LLL * LLL; i++){
	...
	std::vector<std::mutex> list_locks_(max_level_ * LLL * LLL);
	...
	for(int i = max_level_-1; i >= 0; i--){
		...
		for(int jj = 0; jj < vecsize; jj++){
			if(fflag[i][jj] == true) {
		        ...
		        int pos0_ = i * LLL * LLL + element.index[0] * LLL + element.index[1];
        ...
        for(int j = 0; j < vecsize; j++){		
            if(flag_obj[i][j] == true){
				if(fflag[i-1][j] == true){  //the representative exists in the base layer
				    ...
				    pos_ = (i-1) * LLL * LLL + element2.index[0] * LLL + element2.index[1];
	...
	for(int ii = 0; ii < max_level_; ii++){              
		...
		for(int j = 0; j < length[ii]; j++){
			quan_book[ii][j]= new float* [LLL];
		}
			
		for(int j = 0; j < length[ii]; j++){
			for(int l = 0; l < LLL; l++){
				quan_book[ii][j][l]= new float[LLL];
			}	
		}
...
#pragma omp critical
	...
	for(int i = 0; i < max_level_; i++){
		for(int j = 0; j < length[i]; j++){
		    outputQ2.write((char *) merge[i][j], 2 * LLL);
		}
	}
